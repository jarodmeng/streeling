...
I use $P(k \geq 1, n=5)$ to denote the probability of having any four-of-a-kind in a 5-card hand.
...
1. When $0 \leq n \leq 3$, $\underset{0 \leq n \leq 3}P(k \geq 1, n) = 0$.
2. When $4 \leq n \leq 7$, $\underset{4 \leq n \leq 7}P(k \geq 1, n) = \frac{\binom{13}{1} \cdot \binom{48}{n-4}}{\binom{52}{n}}$.
3. When $8 \leq n \leq 11$, $\underset{8 \leq n \leq 11}P(k \geq 1, n) = \frac{\binom{13}{1} \cdot \binom{48}{n-4} - \binom{13}{2} \cdot \binom{44}{n-8}}{\binom{52}{n}}$
...
2. For larger $n$s, the corrections have material impact. $P'(28)$ is close to 60% rather than 100%.
...
where $P(k,n)$ denotes having exactly $k$ sets of four-of-a-kind in an $n$-card hand.

### First case: $4 \leq n \leq 7$

When $4 \leq n \leq 7$, there are at maximum exactly 1 four-of-a-kind in the hand. So $P(n) = P(1,n)$. Then I can use the [two-step approach](#twostep) to solve the problem.
...
is *the raw probability of having at least 1 four-of-a-kind* (it's *raw* because it hasn't been corrected for [duplication](#duplication-and-overcounting)), and
...
\end{aligned}
$$
The most difficult term to compute is $\underset{12 \leq n \leq 15}P(k=1,n)$. Using the same logic as above, I have
...
\end{aligned}
$$
The formula is now a monstrosity. But the process of deriving the formula gives me some hint at how a generalized formula might be worked out. 
...
\end{aligned}
$$
Let $F$ be a function that corresponds to $P(k \geq 1, n)$. That is, a function that takes in an input of $R$ and $n$, and outputs the probability of having (any) four-of-a-kind in an $n$-card hand.
...
I was staring at the [third case solution](#third-case-formula) and realized that the terms can be simplified.
...
$$
This means that the formula can be dramatically simplified to
...
The alternating signs of the second and third terms immediately reminded me of the [inclusion-exclusion principle][4] technique in combinatorics. I wonder if the general formula can be as simple as the following.
...
[4]: https://www.wikiwand.com/en/Inclusion%E2%80%93exclusion_principle