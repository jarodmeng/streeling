...
$$
P(\text{exactly 1 four-of-a-kind}) = P(\text{1 four-of-a-kind}) \cdot [1 - P_{12}(\text{four-of-a-kind})]
$$
...
$$
P_{12}(\text{four-of-a-kind}) = \frac{\binom{12}{1} \cdot \binom{44}{n-8}}{\binom{48}{n-4}}
$$

* $P(\text{1 four-of-a-kind})$ is the probability of having at least 1 four-of-a-kind.
* $P_{12}(\text{four-of-a-kind})$ is *the probability of having four-of-a-kind in the remaining 48 (i.e., only 12 ranks left and hence the subscript) cards when a kind has already been drawn*. When $8 \leq n \leq 11$, $4 \leq n-4 \leq 7$, so the only possibility of having four-of-a-kind is to have exactly 1 four-of-a-kind in the remaining $n-4$ cards in the hand.
...
$$
P(\text{exactly 2 four-of-a-kind}) = P(\text{2 four-of-a-kind}) \cdot [1 - P_{11}(\text{four-of-a-kind})]
$$
...
$$
P_{11}(\text{four-of-a-kind}) = \frac{\binom{11}{1} \cdot \binom{40}{n-12}}{\binom{44}{n-8}}
$$
...
$$
P_{12}(\text{four-of-a-kind}) = P_{12}(\text{exactly 1 four-of-a-kind}) + P_{12}(\text{exactly 2 four-of-a-kind})
$$

Following the same logic, I can get

$$
P_{12}(\text{four-of-a-kind}) = \frac{\binom{12}{1} \cdot \binom{44}{n-8} - \binom{12}{2} \cdot \binom{40}{n-12}}{\binom{48}{n-4}}
$$
...
$$
\begin{aligned}
P(\text{four-of-a-kind}) =\ & P(\text{exactly 3 four-of-a-kind}) +\\
&P(\text{exactly 2 four-of-a-kind}) + \\
&P(\text{exactly 1 four-of-a-kind}) \\
...
\end{aligned}
$$
The formula is now a monstrosity. But the process of deriving the formula gives me some hint at how a generalized formula might be worked out. The general idea is very simple.

$$
P(\text{exactly }k\text{ four-of-a-kid}) = P(k\text{ four-of-a-kind}) \cdot [1 - P_{13-k}(\text{four-of-a-kind})]
$$

Let $G$ be a function that inputs of $r$ (i.e., how many full ranks are in the deck or remaining of the deck) and gives the probability of having at least $k$ four-of-a-kind in an $n$-card hand.

$$
\begin{aligned}
G(r, k, n) &= P_{r}(k\text{ four-of-a-kind in }n\text{-card hand}) \\
&= \frac{\binom{r}{k} \cdot \binom{4r-4k}{n-4k}}{\binom{4r}{n}}
\end{aligned}
$$

Let $F$ be a function that takes in an input of $r$ and gives the probability of having four-of-a-kind in an $n$-card hand.

$$
F(n, r) = P_{r}(\text{four-of-a-kind in }n\text{-card hand})
$$

Then the generalized question becomes the following (assuming the cards are drawn from a standard 13-rank deck).

$$
P(\text{four-of-a-kind}) = F(n, 13)
$$
...
$$
\begin{aligned}
P(\text{exactly 3 four-of-a-kind}) &= P(\text{3 four-of-a-kind}) \cdot [1 - P_{10}(\text{four-of-a-kind})] \\
...
\begin{aligned}
P(\text{four-of-a-kind}) &= F(n, 13) \\
&= P(\text{exactly 1 four-of-a-kind}) + P(\text{exactly 2 four-of-a-kind}) + P(\text{exactly 3 four-of-a-kind}) \\
&= G(13, 1, n) \cdot [1 - F(n-4, 12)] + G(13, 2, n) \cdot [1 - F(n-8, 11)] + G(13, 3, n) \cdot [1 - F(n-12, 10)]
\end{aligned}
$$
Generalizing $F(n, 13)$ becomes
...
df.sim %>%
  ...
  pivot_longer(p.sim:p.formula, names_to = "type", values_to = "probability") %>%
...
The results match the simulation very well. Out of curiosity, I plot the original and corrected results for $n \in [0, 28]$ with the formula results to see which one is "more accurate".

```{r}
df3 <- tibble(
  n = 0:28,
  p.original = map_dbl(n, prob1),
  p.corrected = map_dbl(n, prob2),
  p.formula = map_dbl(n, ~F(.x, r = 13))
)
df3 %>%
  pivot_longer(-n, names_to = "type", values_to = "probability") %>%
  ggplot(aes(x = n, y = probability)) +
    geom_point(aes(color = type)) +
    scale_x_continuous(breaks = seq(0, 28, 1)) +
    scale_y_continuous(label = percent) +
