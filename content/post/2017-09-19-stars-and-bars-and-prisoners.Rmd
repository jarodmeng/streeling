...
What I need here is a way to [count multisets][1]. Formally, the number of multisets of cardinality $k$, with elements taken from a finite set of cardinality $n$, is the multiset coefficient, written as $\left(\!\!{n\choose k}\!\!\right)$.
...
I can also represent those multisets in another form.
...
One needs $n-1$ bars to represent a unique set of cardinality $n$. If I count the total number of dots and bars in a $k$-element multiset, it's $k+n-1$. Then the total number of possible $k$-element multisets equals to the total number of ways of placing $n-1$ bars in $k+n-1$ items, which is ${{k+n-1}\choose{n-1}} = {{k+n-1}\choose{k}}$.

## The denominator

Let's return to the original question. What's the probability of all prisoners being taken to the room at least once after the warden takes $k$ times? To compute this probability, I need to compute the total number of possible scenarios which will serve as the denominator.

The total number should equal to $\left(\!\!{n\choose k}\!\!\right)$, for a $k \geq n$. From the previous section, I can work out $\left(\!\!{n\choose k}\!\!\right) = {{k+n-1}\choose{k}}$.
...
Once I have both the denominator and numerator, the probability is straightforward to compute.
...
I can plot this probability function for $100 \leq k \leq 200$.
...
I can extend the possible $k$ values.

```{r}
curve(Pr, from = 100, to = 30000, xlab = "k", ylab = "Pr(100,k)")
```
...
I can also find out the minimum $k$ that makes the probability greater than 50% (i.e., better than a coin flip).
...
Alternatively, I can simply plug in $n=3, k=3$ into the formula.
...
Is the probability really 10%? I can run a simulation to find out.
...
The simulation suggests that `r paste0(round(p*100, 1), "%")` of the samples have all 3 unique elements in them. That's a lot higher than the 10% I've computed.
